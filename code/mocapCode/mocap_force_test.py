...
class MocapData:
    ...
    def update(self):
        self.penpos = self.mocap.penpos
        self.penori = self.mocap.penori
        self.headpos = self.mocap.headpos
        self.headori = self.mocap.headori
        #print('ponpos','{:7.4f}'.format(self.penpos[0]))

    def display(self):
        # モーションキャプチャデータを表示
        print(f"Pen Position: {self.penpos}")
        print(f"Pen Orientation: {self.penori}")
        print(f"Head Position: {self.headpos}")
        print(f"Head Orientation: {self.headori}")
        print("-" * 50)


class sendCoinput_mocapdata:
    def __init__(self):
        self.mocap=simToReal()
        ...
        self.flag=False
        self.counter=0
        self.mocap.run()#penの座標？をプリントし，natnetのrunの処理を行う
    
  
    
    def getmocapdata(self):#moaptodataからpenとheadのpositionを取得
        self.penpos=self.mocap.penpos
        self.penori=self.mocap.penori
        self.headpos=self.mocap.headpos
        self.headori=self.mocap.headori
    ...
    def run(self):
        self.start_time=time.perf_counter()
        self.jikan=time.perf_counter()-self.Starttime
        self.getmocapdata()
       
        self.end_time=time.perf_counter()
        #time.sleep(0.01-(-self.start_time+self.end_time))
...
mocap_data2 = sendCoinput_mocapdata()
# パラメータ設定
cutoff_frequency = 5  # カットオフ周波数 5Hz
sampling_frequency = 100  # サンプリング周波数 100Hz
m = 10/1000#エンドエフェクタとペンシルの重さ,本当は5グラムだが＋5グラムほどの値を入れると綺麗に重力補償できる
prev_x = 0.0
prev_y = 0.0
prev_z = 0.0
pensaki_prex = 0.0
    ...
    while True:
        ...
        if current_time - last_display_time >= 0.009:#0.009秒間隔でモーキャプの値と最新のセンサの値を取得
            ...
            if latest_data is not None:
                ...
                force_magnitude = math.sqrt(LPF_Fcx**2+LPF_Fcy**2+LPF_Fcy**2)
